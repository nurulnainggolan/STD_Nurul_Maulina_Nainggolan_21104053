...
void insertLast(List &L, address P) {
    if (L.first == nullptr) {
        ...
        Q->next = P; // Set elemen terakhir mengarah ke elemen baru
        P->next = nullptr; // Pastikan elemen baru memiliki next nullptr
    }
}
...
int main() {
    ...
    // Masukkan elemen ke list
    insertFirst(L, p1);   // Ali
    insertLast(L, p2);    // Bobi
    insertLast(L, p3);    // Cindi
